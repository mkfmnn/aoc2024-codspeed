use std::simd::{cmp::{SimdPartialEq, SimdPartialOrd}, i8x64, num::SimdUint};
...
#[inline(always)]
fn set_station(stations: &mut [i8; 512], station_count: &mut [usize; 64], station: usize, bytes_offset: usize) {
    debug_assert!((1..=62).contains(&station));
    let (x, y) = ((bytes_offset % LINE_LEN) as i8, (bytes_offset / LINE_LEN) as i8);
...
fn inner<const REPEAT: bool>(bytes: &[u8]) -> usize {
    ...
    let mut station_count = [0usize; 64];
    {
        ...
        for c in batch {
            ...
            let d = c.simd_gt(U8xRead::splat(b'9')).select(U8xRead::splat(b'A' - 1 - 10), U8xRead::splat(b'0' - 1));
            let d = c.simd_gt(U8xRead::splat(b'Z')).select(U8xRead::splat(b'a' - 1 - 36), d);
    ...
    for i in 0..4 {
        for j in i+1..4 {
            ...
            let n1x = bx + dx;
            let n1y = by + dy;
            let mut m = (n1x.simd_ge(min) & n1x.simd_lt(max) & n1y.simd_ge(min) & n1y.simd_lt(max)).to_bitmask();
            ...
            let n2x = ax - dx;
            let n2y = ay - dy;
            let mut m = (n2x.simd_ge(min) & n2x.simd_lt(max) & n2y.simd_ge(min) & n2y.simd_lt(max)).to_bitmask();
